...
fn get_neighbours(point: &Point, grid: &[Vec<char>]) -> Vec<Point> {
    ...
    if point.row > 0 {
        ...
        if grid[point.row][point.column] == '.' || grid[point.row][point.column] == '^' {
            result.push(point);
        }
    }
    if point.row < row_len - 1 {
        ...
        if grid[point.row][point.column] == '.' || grid[point.row][point.column] == 'v' {
            result.push(point);
        }
    }
    if point.column > 0 {
...
fn find_all_paths(
    ...
    while let Some((to_visit, path, current_distance)) = to_visit_queue.pop() {
        ...
        if path[0..(path.len() - 1)].contains(&to_visit) {
            continue;
        }

        if destinations.is_empty() && to_visit == end_point {
