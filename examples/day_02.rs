...
fn main() {
    ...
    let result: usize = input
        ...
        .filter(|(_, game_data)| {
            game_data.iter().all(|single_grab| {
                *single_grab.get(&Color::Red).unwrap_or(&0) <= 12
                    && *single_grab.get(&Color::Green).unwrap_or(&0) <= 13
                    && *single_grab.get(&Color::Blue).unwrap_or(&0) <= 14
            })
        })
        .inspect(|(game_id, _)| println!("Passing game id is {game_id}"))
